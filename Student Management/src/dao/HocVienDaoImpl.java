...
public class HocVienDaoImpl implements HocVienDAO {
	...
	public int createOrUpdate(HocVien hocVien) {
		try {
			Connection conn = ConnectDB.getConnection();
			String sql = "INSERT INTO HOCVIEN(ma_hoc_vien, ho_ten, ngay_sinh, gioi_tinh, so_dien_thoai, dia_chi, tinh_trang) VALUES(?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE ho_ten = VALUES(ho_ten), ngay_sinh = VALUES(ngay_sinh), gioi_tinh = VALUES(gioi_tinh), so_dien_thoai = VALUES(so_dien_thoai), dia_chi = VALUES(dia_chi), tinh_trang = VALUES(tinh_trang);";
	...
	public int delete(HocVien hocvien) {
		try {
			Connection conn = ConnectDB.getConnection();
			String sql = "DELETE FROM HOCVIEN WHERE ma_hoc_vien=?";
			PreparedStatement ps = conn.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS);
			ps.setInt(1, hocvien.getMa_hoc_vien());
			ps.executeUpdate();
			ResultSet rs = ps.getGeneratedKeys();
